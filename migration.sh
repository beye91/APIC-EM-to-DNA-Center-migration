...
curl --header "Content-Type:application/json" --header "Accept:application/json" --header "x-auth-token:${DNAC_TOKEN}" -X POST --data '{"type":"building","site":{"building":{"name":"Building 1","parentName":"Global/'${area}'","latitude":"37.413080","longitude":"-121.933790"}}}' --insecure -s https://${DNAIP}/dna/intent/api/v1/site 2>&1 | jq '.' 
sleep 5
